BASTION_KEY_FILE=$(cat $PROJECT_DIR/.env_BASTION_KEY_FILE)
log BASTION_KEY_FILE $BASTION_KEY_FILE
...
TUNNEL_ID=$(ps aux | grep ssh | grep $BASTION_PUBLIC_DNS | tr -s ' ' | cut -d ' ' -f 2)
